...
// База данных сохраняется в /tmp на Render (диск постоянный)
...
db.serialize(() => {
    ...
    // Добавляем системных пользователей (если ещё нет)
...
app.post('/register-attempt', (req, res) => {
    ...
    db.get(`SELECT username FROM users WHERE username = ?`, [username], (err, row) => {
        ...
        db.run(`INSERT INTO users (username, password) VALUES (?, ?)`, [username, password], (err) => {
            if (err) return res.json({ success: false, error: 'Ошибка БД: ' + err.message });
