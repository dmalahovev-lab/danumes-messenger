...
io.on('connection', (socket) => {
    ...
    function sendUsersList() {
        let list = Object.keys(db.users).map(name => {
    ...
    socket.on('register_user', (data) => {
        ...
        saveDB();
        socket.emit('auth_success_msg', 'Регистрация успешна! Теперь вы можете войти.');
    });
    ...
    socket.on('send_direct_message', (data) => {
        ...
        const targetSocketId = Object.keys(activeConnections).find(key => activeConnections[key] === data.toUser);
        if (targetSocketId) {
            io.to(targetSocketId).emit('receive_direct_message', { from: username, msg: messagePayload });
        }
        // Подтверждение отправителю
        socket.emit('message_sent_confirm', { to: data.toUser, msg: messagePayload });
    });
    ...
    socket.on('delete_message', (data) => {
        ...
        if (db.messages[roomKey]) {
            ...
            const targetSocketId = Object.keys(activeConnections).find(key => activeConnections[key] === data.toUser);
            if (targetSocketId) {
                io.to(targetSocketId).emit('message_deleted_sync', { from: username, msgId: data.msgId });
            }
            socket.emit('message_deleted_sync', { from: data.toUser, msgId: data.msgId });
        }
    });
