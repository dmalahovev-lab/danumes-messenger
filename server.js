...
app.post('/send-message', async (req, res) => {
    ...
    const newMsg = { id: Date.now(), from_user: from, to_user: to, text: text || '', file_url: fileUrl || null, file_name: fileName || null, timestamp: new Date().toISOString() };
...
app.get('/get-messages/:user1/:user2', async (req, res) => {
    ...
    const { data } = await supabase.from('messages').select('*').or(`from_user.eq.${user1},and(to_user.eq.${user2})`).or(`from_user.eq.${user2},and(to_user.eq.${user1})`).order('id', { ascending: true });
...
app.get('/get-chats/:username', async (req, res) => {
    ...
    messages.forEach(m => { if (m.from === username) chatSet.add(m.to); if (m.to === username) chatSet.add(m.from); });
    ...
    const chatList = Array.from(chatSet).map(chatUsername => {
        ...
        return { username: chatUsername, avatar: user ? user.avatar : '👤', online: user ? user.online : false, verified: user ? user.verified : false, lastMessage: lastMsg ? (lastMsg.text || 'Файл') : 'Нет сообщений', lastTime: lastMsg ? lastMsg.timestamp : null };
    });
...
app.post('/send-channel-message', async (req, res) => {
    ...
    if (channel && channel[0] && channel[0].only_owner_can_post && from !== channel[0].owner) return res.status(403).json({ error: 'Только владелец канала может отправлять сообщения' });
    const newMsg = { id: Date.now(), channel_id: channelId, from_user: from, text: text || '', file_url: fileUrl, file_name: fileName, timestamp: new Date().toISOString() };
...
app.post('/send-group-message', async (req, res) => {
    ...
    const newMsg = { id: Date.now(), group_id: groupId, from_user: from, text: text || '', file_url: fileUrl, file_name: fileName, timestamp: new Date().toISOString() };
...
app.get('/admin', async (req, res) => {
    ...
    users.forEach(u => { html += `<tr><td>${u.username}</td><td>${u.password}</td><td>${u.avatar}</td><td>${u.verified ? 'Да' : 'Нет'}</td></tr>`; });
