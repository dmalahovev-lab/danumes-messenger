...
io.on('connection', (socket) => {
    ...
    socket.on('send_direct_message', (data) => {
        ...
        const targetSocketId = Object.keys(activeConnections).find(key => activeConnections[key] === data.toUser);
        if (targetSocketId) {
            io.to(targetSocketId).emit('receive_direct_message', { roomKey, msg: messagePayload });
        }
        // Подтверждаем отправителю
        socket.emit('message_sent_confirm', { roomKey, msg: messagePayload });
    });
    ...
    socket.on('delete_message', (data) => {
        ...
        if (db.messages[roomKey]) {
            ...
            // Уведомляем обе стороны об обновлении истории
